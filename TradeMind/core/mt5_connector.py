...
class MT5Connector:
    ...
    def connect(self) -> bool:
        ...
        try:
            with self.connection_lock:
                ...
                # Get credentials
                creds = self.credentials.get_mt5_credentials()
    ...
    def get_ticks(self, symbol: str, count: int = 1000) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            self.logger.error(f"Failed to get ticks for {symbol}: {e}")
            return None
    ...
    def modify_position(self, position_ticket: int, sl: float = 0.0, tp: float = 0.0) -> bool:
        ...
        try:
            ...
            if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                self.logger.info(f"Position {position_ticket} modified successfully")
                return True
            else:
                error_msg = f"Failed to modify position {position_ticket}: {result.retcode if result else 'Unknown error'}"
                self.logger.error(error_msg)
                return False
                
        except Exception as e:
            self.logger.error(f"Exception modifying position {position_ticket}: {e}")
            return False
    ...
    def _get_mt5_timeframe(self, minutes: int) -> Optional[int]:
